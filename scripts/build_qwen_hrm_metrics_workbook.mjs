...
notes.getRange("A1:B23").values = [
  ...
  ["ARC-Easy transfer", "On full ARC-Easy validation, Qwen3-0.6B option-prior calibration transfers from 348/570 to 355/570 and agreement_blend preserves 355/570. On ARC-Easy validation[:200], Qwen3-1.7B answer-prior weight 1.0 ties base at 173/200, while the ARC-Challenge-tuned weight 1.7 drops to 172/200."],
...
notes.getRange("A1:B23").format.wrapText = true;
notes.getRange("A1:B23").format.autofitColumns();
...
await workbook.render({ sheetName: "ARC Runs", range: "A1:R50", scale: 2 });
